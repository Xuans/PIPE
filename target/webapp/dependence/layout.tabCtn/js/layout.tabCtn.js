                ...
                if(tabWidget && tabWidget.href && tabWidget.href()==='layout.tabCtn'){
                    ...
                    updateTabDataModel=function(){
                        ...
                        tabs.map(function (elem, index) {
                            var cttWidgets = tabWidget.children(':active');
                    ...
                    switch(type){
                        ...
                        case widget.STATUS.WIDGET_UPDATE:
	                        ...
	                        if (cttWidgets.length < tabs.length) {
		                        append = function (cttWidget) {
                                    ...
                                    cttWidget.attr('uuid',tabs[tabs.length-1].uuid)
			                        cttWidgets = tabWidget.children(':active');
		                        ...
		                        if (tabs.length < 2) {
                                    ...
                                    for(var i=0;i<tabs.length;i++){
                                            var item = tabs[i];
                                            tabsMap[item.uuid]=item;

                                        }

                                    for(var j=0;j<cttWidgets.length;j++){
                                        var widgetItem = cttWidgets[j],

                                            uuid=widgetItem.data.attr.uuid;

                                            if(!tabsMap.hasOwnProperty(uuid)){
                                                cttWidgets.filter(':eq('+j+')').destroy();
                                            }
                        ...
                        case widget.STATUS.WIDGET_APPEND:
                            ...
                            cttWidgets = tabWidget.children(':active');
                            cttWidgets.each(function(index){
                                var item = cttWidgets.eq(index),
                                    uuid = tabs[index].uuid;
